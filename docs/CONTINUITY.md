...
- Date: 2026-07-19
- GitHub Issue: `#110` - https://github.com/VietCT04/TicketPass/issues/110
- Summary: Implemented database-side public event browse filters for optional literal case-insensitive `q`, exact normalized `city`, and explicit-offset event-time bounds. The grouped aggregate and matching count query share the same nullable predicates; existing eligibility, safe projection, ordering, and response shape remain unchanged. Public pagination now accepts up to 100 items, including the existing event-detail route that shares this parser. No migration, frontend controls, ranking, or search infrastructure was added; browser controls remain issue `#111`.
- Files changed:
  - `apps/api/src/main/java/com/ticketpass/api/listing/EventBrowseController.java`
  - `apps/api/src/main/java/com/ticketpass/api/listing/EventBrowseFilters.java`
  ...
  - `docs/CONCERNS.md`
  - `docs/user-stories/US-0015-search-filter-events.md`
  - `docs/CONTINUITY.md`

- Date: 2026-07-19
- GitHub Issue: `#95` - https://github.com/VietCT04/TicketPass/issues/95
- Summary: Defined the documentation-only buyer receipt-confirmation and settlement-release contract. Only the authenticated buyer can make the immutable post-transfer confirmation; it records before provider execution and never itself proves release. A future one-to-one durable release operation uses a stable idempotency key, lease-backed recovery, provider lookup, and final locked confirmation before held settlement can move to `RELEASED_TO_SELLER`. The contract also defines safe responses, error behavior, audit boundaries, and local-only mock settlement limits. Backend implementation remains issue `#96`, browser work remains `#97`, and timeout/review handling remains `#98` and `#99`; no migration, application code, or verification command was run.
- Files changed:
  - `docs/API.md`
  - `docs/DATABASE.md`
  - `docs/SECURITY.md`
...
- Date: 2026-07-19
- GitHub Issue: `#145` - https://github.com/VietCT04/TicketPass/issues/145
- Summary: Defined the documentation-only administrative event-request review contract. Future persisted `USER`/`ADMIN` authority, exact normalized event identity, immutable terminal request outcomes, transactional sibling resolution, minimal audit events, private queue/tracking responses, and the approved-request seller continuation are specified. Implementation remains intentionally split across issues `#146` through `#151`; no migration, backend, frontend, or verification command was run.
- Files changed:
  - `docs/API.md`
  - `docs/DATABASE.md`
  - `docs/SECURITY.md`
  - `docs/flows/ADMIN_EVENT_REQUEST_REVIEW_FLOW.md`
  - `docs/flows/SELLER_LISTING_FLOW.md`
  - `docs/user-stories/US-0008-request-missing-event.md`
  - `docs/user-stories/US-0024-admin-reviews-event-requests.md`
  - `docs/CONCERNS.md`
  - `docs/CONTINUITY.md`
