...
Issue `#65` defines the checkout security contract for `US-0007`. Issue `#66` implements core provider-neutral order persistence. Issue `#67` implements authenticated checkout preparation and an isolated mock hosted provider; webhooks, order completion, inventory sale transitions, and frontend checkout recovery remain unimplemented.
...
- Provider replay/deduplication records and provider references are restricted operational payment records. They are not automatically added to generic `audit_events`; broader payment audit access and retention are deferred to issue `#70`. The mock event outbox is stored but not delivered in this issue; its future HMAC uses a configured secret and belongs to `#68`.
