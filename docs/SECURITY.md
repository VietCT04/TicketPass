...
Issue `#65` defines the checkout security contract for `US-0007`. Issues `#66` and `#67` implement persistence and mock hosted checkout. Issue `#68` implements signed mock webhook delivery, database-backed replay protection, and verified sale completion; browser checkout recovery remains deferred.

- `POST /api/reservations/{reservationId}/checkout` requires authenticated server-side session validation and is explicitly protected by `SecurityConfig`. Buyer ownership is derived from `AuthenticatedUser`, never from browser input. `GET /api/orders/{orderId}` remains deferred.
...
- The public `/mock-provider/**` routes do not require a TicketPass login cookie and display only amount, currency, server expiry, and mock-provider state. They never receive or expose TicketPass session data, seller/buyer identity, event or seat details, ticket data, private transfer details, payment credentials, or provider secrets. Their success, failure, and cancellation redirects are presentation hints only.
- `POST /api/payments/webhooks/mock` is the sole public payment API exception in `SecurityConfig`. It receives exact raw bytes, verifies the configured HMAC-SHA256 signature and five-minute timestamp window before JSON parsing, compares signatures in constant time, and returns controlled `401` responses without verification detail. It never logs request bodies, signatures, secrets, payment values, identities, or ticket data.
- The receiver ledger uses an atomic database uniqueness insert for `(MOCK, provider_event_id)`. Duplicates return `200` without repeating marketplace effects. The outbox uses configured-only webhook URLs and bounded retries; only `500` responses cause retry.
